...
export const JobDashboard = () => {
  ...
  // Example skillset checkboxes, replace these with the actual skills as needed
  const skillsetCheckboxes = [
    { id: 1, label: 'Skill 1' },
    { id: 2, label: 'Skill 2' },
    { id: 3, label: 'Skill 3' },
    { id: 4, label: 'Skill 4' },
  ...
  const totalCheckboxes = skillsetCheckboxes.length;
        ...
        <div className="relative py-96 md:py-96 lg:py-96 z-10">
          <div className="p-4 w-full max-w-2xl mx-auto bg-gradient-to-r from-pink-500 via-red-500 to-yellow-500 rounded-lg border border-violet-300/35 shadow-[0_0_80px_inset] shadow-violet-300/50">
            ...
            <div className="bg-black text-white rounded-lg py-4 px-4 flex flex-col gap-2 border border-violet-300/35 shadow-[0_0_80px_inset] shadow-violet-300/50">
              {skillsetCheckboxes.map((checkbox) => (
                <div
                  key={checkbox.id}
            ...
            {/* Display percentage */}
            <p className="mt-2 text-sm text-white">{`${checkedCount} out of ${totalCheckboxes} skills checked (${percentage.toFixed(1)}%)`}</p>
          </div>
        </div>
      </section>
    </>
